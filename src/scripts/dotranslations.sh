...
rm -f res/*.qm
$QT_STATIC/bin/lrelease zecwallet.pro
...
ls res/*.qm | awk -F '[_.]' '{print $4}' | while read -r language ; do 
    if [ -f $QT_STATIC/translations/qtbase_$language.qm ]; then
        $QT_STATIC/bin/lconvert -o res/zec_$language.qm $QT_STATIC/translations/qtbase_$language.qm res/zecwallet_$language.qm
        mv res/zec_$language.qm res/zecwallet_$language.qm
    fi
done
